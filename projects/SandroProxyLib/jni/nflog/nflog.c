...
void *threadClientCommand(void *data)
{
	char buf[4];
	int count = read(unix_socket_fd, buf, sizeof(buf));
	if (count == 4){
...
static int write_and_release(char * output){
	printf(output);
	int len = strlen(output);
	int res = 0;
	if (len > 0){
		res = write(unix_socket_fd, output,len);
	}
	free(output);
	if ( res < 0){
		printf("Error on writing to unix socket \n");
		exit(EXIT_FAILURE);
	}
}
static int log_cb(const struct nlmsghdr *nlh, void *data)
{
    ...
    if (tb[NFULA_PREFIX]) {
        ...
        asprintf(&output, "%s ", prefix);
        write_and_release(output);
    }
    ...
    if (tb[NFULA_IFINDEX_INDEV]) {
        ...
        asprintf(&output, "IN=%s ", instr ? instr : "");
    } else {
        asprintf(&output, "IN= ");
    }
    write_and_release(output);
    ...
    if (tb[NFULA_IFINDEX_OUTDEV]) {
        ...
        asprintf(&output, "OUT=%s ", outstr ? outstr : "");
    } else {
        asprintf(&output, "OUT= ");
    }
    write_and_release(output);
    ...
    if (tb[NFULA_PAYLOAD]) {
        ...
        asprintf(&output, "SRC=%u.%u.%u.%u DST=%u.%u.%u.%u ",
        ...
        asprintf(&output, "LEN=%u ", ntohs(iph->tot_len));
        write_and_release(output);
        ...
        switch(iph->protocol) 
        {
            case IPPROTO_TCP: 
                {
                    ...
                    asprintf(&output, "PROTO=TCP SPT=%u DPT=%u ",
            ...
            case IPPROTO_UDP:
                {
                    ...
                    asprintf(&output, "PROTO=UDP SPT=%u DPT=%u LEN=%u ",
            ...
            case IPPROTO_ICMP:
                {
                    ...
                    asprintf(&output, "PROTO=ICMP TYPE=%u CODE=%u ",
                        ich->type, ich->code);
                    write_and_release(output);
                    break;
                }
            default: 
                {
                    asprintf(&output, "PROTO=%u ", iph->protocol);
                    write_and_release(output);
                    break;
                }
        }
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 3) {
        printf("Usage: %s [queue_num] [unix_socket_name]\n", argv[0]);
        exit(EXIT_FAILURE);
    }
    qnum = atoi(argv[1]);
    unix_socket_addr = argv[2];

    struct sockaddr_un sockAddr;
	socklen_t sockLen;

	/* connect to unix socket server to get fd */
	if (makeAddr(unix_socket_addr, &sockAddr, &sockLen) < 0){
		perror("makeAddr unix socket");
		exit(EXIT_FAILURE);
	}

	unix_socket_fd = socket(AF_LOCAL, SOCK_STREAM, PF_UNIX);
	if (unix_socket_fd < 0) {
		perror("unix socket socket()");
		exit(EXIT_FAILURE);
	}

	if (connect(unix_socket_fd, (const struct sockaddr*) &sockAddr, sockLen) < 0) {
		perror("unix socket connect()");
		exit(EXIT_FAILURE);
	}

	printf("acting as client on unix socket %s\n", sockAddr.sun_path+1);
	pthread_t thread_id;
	pthread_create(&thread_id, NULL, threadClientCommand, (void *)NULL);
